...
function shootNormal() {
  ...
  p.shotCd = 0.11;
  state.playerBullets.push({ x: p.x, y: p.y - p.h / 2 - 10, w: 8, h: 16, vy: -770, dmg: 1, charge: false });
}

function shootCharge(mult = 1) {
  const p = state.player;
  state.playerBullets.push({
    ...
    h: 28,
    vy: -900,
    dmg: Math.round(6 * mult),
    charge: true,
    pierce: 2 + Math.floor(mult),
  });
  spawnBurst(p.x, p.y - 20, "#6fd4ff", 14, 120);
}
...
function update(dt) {
  ...
  if (keys.Space) {
    ...
    if (p.charge >= 0.35) {
      const mult = 1 + p.charge / p.maxCharge;
      shootCharge(mult);
  ...
  const enemyDrift = 35 + state.phase * 7;
  const wobble = Math.sin(performance.now() * 0.0018) * 22;

  for (const e of state.enemies) {
    e.y += dt * (7 + state.phase * 0.58);
    e.x += dt * enemyDrift * Math.sin((e.y + wobble) * 0.02);
    ...
    if (e.y + e.h / 2 >= p.y - p.h / 2) {
      damagePlayer(22);
      e.hp = 0;
      spawnBurst(e.x, e.y, "#ff6f91", 12, 160);
    }
  }
  ...
  for (const pb of state.playerBullets) {
    if (state.boss && rectHit(pb, state.boss)) {
      state.boss.hp -= pb.dmg;
  ...
  if (state.boss && state.boss.hp <= 0) spawnBurst(state.boss.x, state.boss.y, "#ffce72", 48, 220, 1.2);
  ...
  const waveClear = !state.boss && state.enemies.length === 0;
  const bossClear = state.boss && state.boss.hp <= 0;
...
function render() {
  ...
  if (state.boss) {
    const b = state.boss;
    drawSprite(b.x, b.y, b.w, b.h, SPRITES.boss, { "3": "#ffce72", "4": "#8e2f2f" });
  }
...
function updateHud() {
  ...
  if (state.boss) {
    ...
    bossHpBar.style.width = `${(state.boss.hp / state.boss.maxHp) * 100}%`;
    bossHpText.textContent = `${Math.max(0, Math.ceil(state.boss.hp))} / ${state.boss.maxHp}`;
  } else {
    bossRow.hidden = true;
  }
}
